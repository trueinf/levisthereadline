 ...
 */
export async function callmai(path, payload, { timeoutMs = config.textTimeoutMs } = {}) {
  console.log(path)
  console.log(payload)
  ...
  let upstream
  try {
console.log(`${config.openaiBaseImageUrl}${path}`)
    upstream = await fetch(`${config.openaiBaseImageUrl}${path}`, {
      method: 'POST',
      headers,
